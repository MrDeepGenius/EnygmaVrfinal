...
const PAGE_SIZE = 120;
const GENRES = ["Acción", "Comedia", "Drama", "Ciencia Ficción", "Terror", "Aventura", "Animación", "Thriller", "Romance", "Familia"];

function SectionHero({ items }: { items: Movie[] }) {
  ...
  const tmdbParams = { tmdbId: item?.id ?? "", type: "movie" as const };
  ...
  return (
    <div
      ...
      onClick={() => setLocation(`/detail/movie/${item.id}`)}
    >
      {/* Backdrop */}
      ...
      {/* Content — centered at bottom like HBO Max */}
      <div
      ...
      >
        ...
        <div className="flex items-center gap-2 text-white/65 text-xs font-semibold mb-2">
          {item.edadRecomendada && <span>{item.edadRecomendada}+</span>}
          {item.edadRecomendada && item.año && <span className="w-px h-3 bg-white/30 rounded-full" />}
...
export default function Movies() {
  ...
  const cacheKey = `enygma:movies:first:${profile || "default"}:${genre || "all"}`;
  const cachedFirstPage = readLocalCache<{ items: Movie[]; total: number }>(cacheKey, 24 * 60 * 60 * 1000);

  const { data, isLoading, isFetching } = useListMovies({
  ...
  useEffect(() => { setOffset(0); setItems([]); setHasMore(true); }, [profile, genre]);
  useEffect(() => {
    if (!data) return;
  ...
  const allRaw: Movie[] = items.length > 0 ? items : (offset === 0 ? (data?.items || []) : []);

  const yearOptions = useMemo(() => {
    const years = allRaw.map(m => m.año?.trim()).filter((y): y is string => !!y && /^\d{4}$/.test(y)).map(Number);
  ...
  const allItems = useMemo(() => year ? allRaw.filter(m => m.año?.trim() === year) : allRaw, [allRaw, year]);
    ...
    <Layout>
      ...
      <div className="max-w-screen-2xl mx-auto px-3 md:px-10 lg:px-16 xl:px-20 py-6 bg-black">
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2 md:gap-4">
              {allItems.map(movie => <PosterCard key={movie.id} item={movie} type="movie" variant="grid" />)}
            </div>
            {hasMore && (
              <><div ref={sentinelRef} className="h-1" /><div className="flex justify-center mt-6"><div className="w-5 h-5 border-2 border-[#7B2FBE]/40 border-t-[#7B2FBE] rounded-full animate-spin" /></div></>
            )}
          </>
        ) : (
          <div className="text-center py-20 text-white/30"><p className="text-sm">No se encontraron películas.</p></div>
        )}
      </div>
    </Layout>
  );
